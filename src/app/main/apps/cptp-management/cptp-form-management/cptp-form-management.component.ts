...
@Component(
{
    selector: 'app-cptp-form-management',
    templateUrl: './cptp-form-management.component.html',
    styleUrls: ['./cptp-form-management.component.scss']
})

export class CptpFormManagementComponent implements OnInit
{
    ...
    // For Insert Dummy Data For Clause Category
    ...
    ngOnInit()
    {
        ...
        this.CityRecords();
        // this.DummyDataInsertForClauseCategory();
        this.DummyDataInsertBroker();
    }

    get f() { return this.CPTPManagementForm.controls; }

    DummyDataInsertForClauseCategory() : void
    {
        for (var i=1; i <= 25; i++)
        {
            for (var j=1; j <= 10; j++)
            {
                const req =
                {
                    parentId : i,
                    nos : j,
                    termsName:'Clause Category Terms '+j,
                    // cpFormId:i,
        ...
        for (var i=1; i <= 10; i++)
        {
            for (var j=1; j <= 5; j++)
            {
                const req =
                {
                    ...
                    firstName : 'Broker',
                    lastName : i,
                    email : 'broker'+i+'company'+j+'@email.com',
                    mobileNo : '999991234'+i,
                    businessPhone : '123456',
                    businessFax : '123456',
                    title : 'Broker',
                    address : 'Ahmedabad',
                    password : '$2a$10$5.s9F9mTezCaTsG7IcFjWewtfWihacvPMpYkQFV.z53fWjXSpGUdK',
                    createdBy: localStorage.getItem('userId'),
                    updatedBy: localStorage.getItem('userId'),
                };
                console.log('Insert Data Request As Below');
    ...
    // Set Applicable Charter Party Type Start
        ...
        setTimeInfo(Type): void
        {
            console.log(Type);
            var timeInfo = moment(Type).format('HH:mm:ss');
            console.log(timeInfo);
            this.cpTime = timeInfo;
        }
    ...
    // CP Form Records Fetch Start
        ...
        CityRecords(): void
        {
            try
            {
                this.http.get(`${config.baseUrl}/CityRecords`).subscribe(
                    res =>
                    {
                        this.cityFormList = res;
                        if (this.cityFormList.success)
                        {
                            this.cityFormData = this.cityFormList.data;
                        }
                    },
                    err =>
                    {
        ...
        onCityChangeEvent(event): void
        {
            this.cpCity = event.target.value;
        }
